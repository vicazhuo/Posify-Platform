...
class StoreOrderCartInfoServices extends BaseServices
{
     ...
     * @throws \Psr\SimpleCache\InvalidArgumentException
     */
    public function copyCart(int $oid,$orderNewid)
    {
        $cart =  $this->dao->getOne(['oid' => $oid])->toArray();
        $data =[
                'oid' => $orderNewid,
                'cart_id' =>time(),
                'product_id' => $cart['cart_info']['productInfo']['id'],
                'cart_info' => json_encode($cart['cart_info']),
