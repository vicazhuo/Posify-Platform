...
class YuePayServices extends BaseServices
{
     ...
     * @return bool
     */
    public function yueOrderPayV2(array $orderInfo, $uid)
    {
        $type = 'pay_product';
        ...
        $this->transaction(function () use ($services, $orderInfo, $userInfo, $type) {
            ...
            //拼团返佣写入记录
            $number = ['luck_no' => $orderInfo['luck_no']];
            $res = $userMoneyServices->income('luck_to_payment', $userInfo['uid'], $number, 0, $orderInfo['id']);
            /** @var StoreOrderSuccessServices $orderServices */
            $orderServices = app()->make(StoreOrderSuccessServices::class);
            $res = $res && $orderServices->paySuccessV2($orderInfo, PayServices::YUE_PAY);//余额支付成功
        });
        return ['status' => true];
    }
