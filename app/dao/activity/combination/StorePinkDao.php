...
class StorePinkDao extends BaseDao
{
     ...
     * @return StorePink
     */
    public function updateAll(int $uid,int $cid,array $data){

        return StorePink::update($data,['uid'=>$uid,'cid'=>$cid]);
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function getPinkingOne(array $where)
    {
     ...
     * @return mixed
     */
    public function getPinkUserOnlyList(array $where, $page, $limit)
    {

        return $this->getModel()->where($where)->group('pid')->with('getProduct')->select()->toArray();
