...
use app\services\other\PinkAllocationServices;
use app\services\other\QueeServices;
...
class StorePinkServices extends BaseServices
{
     ...
     * @return mixed
     */
    public function createPinkV2(array $orderInfo)
    {    
        ...
        if ($orderInfo['pink_id']) {
            ...
            if ($this->isPinkBe($pink, (int)$orderInfo['pink_id'])) {
                return false;
            }
            ...
            $pink['k_id'] = (int)$orderInfo['pink_id'];//拼团id
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function getLastOrder($uid,$cid)
    {
       return $this->dao->getPinkingOne(['uid'=>$uid,'cid'=>$cid,'status'=>2]);
    }
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function getUserPinkOkList(int $uid,$page,$limit)
    {
        $map =['uid'=>$uid];
        return $this->dao->getPinkUserOnlyList($map,$page,$limit);
    }
     ...
     * @return string
     */
    public function setIsFoloow($uid, $cid,$is_follow)
    {
        return $this->dao->updateAll($uid,$cid,['is_follow'=>$is_follow]);
    }
}
