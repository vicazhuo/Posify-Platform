...
class UserExtractServices extends BaseServices
{
    ...
    {
        $fail_time = time();
        $extract_number = floatval($userExtract['extract_price']);
        $charge_fee = sys_config("service_charge");
        if(floatval($charge_fee)>0){
            $percent = 100-$charge_fee;
            $extract_number =  ($extract_number/$percent)*$charge_fee;
        }
        $mark = '提现失败,退回佣金' . $extract_number . '元（含手续费）';
        $uid = $userExtract['uid'];
        $status = -1;
    ...
    public function bank(int $uid)
    {
        ...
        $data['sCharge'] = sys_config('service_charge');//提现最低金额
        return $data;
    }
    ...
    public function cash(int $uid, array $data)
    {
        ...
        $openid = '';
        $service_charge = sys_config('service_charge');//提现手续费率
        $total_fee = (floatval($data['money'])/100)*$service_charge;
        $extract_price = $data['money']-$total_fee;
        $insertData = [
            'uid' => $user['uid'],
        ...
        if ($data['extract_type'] == 'alipay') {
            ...
            $mark = '使用支付宝提现' . $data['money'] . '元'.'，扣除手续费'.$total_fee.'元';
        } else if ($data['extract_type'] == 'bank') {
            $mark = '使用银联卡' . $insertData['bank_code'] . '提现' . $data['money'] . '元'.'，扣除手续费'.$extract_price.'元';
        } else if ($data['extract_type'] == 'weixin') {
            $insertData['qrcode_url'] = $data['qrcode_url'];
            $mark = '使用微信提现' . $data['money'] . '元'.'，扣除手续费'.$total_fee.'元';
            /** @var WechatUserServices $wechatServices */
            $wechatServices = app()->make(WechatUserServices::class);
