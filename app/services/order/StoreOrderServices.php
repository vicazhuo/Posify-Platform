...
HTML;
    ...
    public function coptyOrder(int $id, array $pinkOrder)
    {
        $orderInfo = $this->dao->getOne(['id' => $id, 'is_del' => 0])->toArray();
        /** @var StoreOrderCreateServices $storeOrderCreateServices */
        $storeOrderCreateServices = app()->make(StoreOrderCreateServices::class);
        $orderInfo['order_id'] = $storeOrderCreateServices->getNewOrderId('fl');
        $orderInfo['add_time'] = time();
        $orderInfo['status'] = 0;
        $orderInfo['paid'] = 0;
        $orderInfo['pink_id'] = 0;
        $orderInfo['unique'] = md5(time());
        unset($orderInfo['id']);
        /** @var StorePinkServices $pinkService */
        $pinkService = app()->make(StorePinkServices::class);
        list($pink, $pindAll) = $pinkService->getPinkList($orderInfo['combination_id'], true);//拼团列表
        if (is_array($pink) && count($pink) > 0) {
            $max = 100000;
            $pk = [];
            foreach ($pink as $k => $v) {
                if ($v['count'] < $max) {
            ...
            $orderInfo['pink_id'] = $pk['id'];
        }
        $od = $this->dao->save($orderInfo);
        $orderInfo['id'] = $od->id;
        /** @var StoreOrderCartInfoServices $cartServices */
        $cartServices = app()->make(StoreOrderCartInfoServices::class);
        $cart = $cartServices->copyCart($id,$orderInfo['id']);
        /** @var YuePayServices $yueServices */
        $yueServices = app()->make(YuePayServices::class);
        $pay = $yueServices->yueOrderPay($orderInfo, $pinkOrder['uid']);
        if ($pay['status']) {
...
//                }
     ...
     * @return string
     */
    public function setIsFoloow($pink_id,$order_id,$is_follow){

        $res = $this->dao->update($order_id,['is_follow'=>$is_follow]);
         /** @var StorePinkServices $storePinkService */
         $storePinkService = app()->make(StorePinkServices::class);
         $storePinkService->setIsFoloow($pink_id,$is_follow);
