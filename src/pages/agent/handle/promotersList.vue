        ...
        <Modal v-model="modals"  scrollable  footer-hide closable :title="listTitle === 'man'?'统计推广人列表':'推广订单'" :mask-closable="false"  width="900" @on-cancel="onCancel">
            <div class="table_box">
                ...
                class="tabform" @submit.native.prevent>
                    ...
                    <FormItem label="用户类型：">
                        <Select v-model="formValidate.type" style="width:250px">
                            <Option v-for="(item,i) in fromList.fromTxt2" :key="i" :value="item.val">{{ item.text }}</Option>
                        </Select>
                    </FormItem>
                    <FormItem label="搜索：">
                        <Input placeholder="请输入请姓名、电话、UID" v-model="formValidate.nickname" style="width:250px;"></Input>
                    </FormItem>
                    <FormItem label="订单号：">
                        <Input placeholder="请输入请订单号" v-model="formValidate.order_id" style="width:250px;margin-right:14px;"></Input>
