...
class StoreCombinationServices extends BaseServices
{
    ...
    public function getInfo(int $id)
    {
        ...
        $info['is_auto'] = (boolean) $info['is_auto'];
...
//            sort($item['detail'], SORT_STRING);
    ...
    public function checkoutProductStock(int $uid, int $combinationId, int $cartNum = 1, string $unique = '')
    {
        ...
        if (!$StoreCombinationInfo) {
            throw new ValidateException('该商品已下架或删除-4');
        }
        ...
        if ($cartNum > $res['quota']) {
            throw new ValidateException('该商品库存不足a' . $cartNum);
        }
        ...
        if ($product_stock < $cartNum) {
            throw new ValidateException('该商品库存不足b' . $cartNum);
        }
        return $unique;
    }
}
