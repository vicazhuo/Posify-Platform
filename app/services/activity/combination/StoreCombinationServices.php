...
//            sort($item['detail'], SORT_STRING);
    ...
    public function getCombinationListV2($uid)
    {
        [$page, $limit] = $this->getPageValue();
        $list = $this->dao->combinationListV2(['is_del' => 0, 'is_show' => 1, 'pinkIngTime' => true, 'storeProductId' => true], $page, $limit);
        /** @var UserFollowServices $follow */
        $follow = app()->make(UserFollowServices::class);
        $data=[];
        foreach ($list as &$item) {
            ...
            if(!empty($item['lid'])){
                $t =  $item['next_time'];
                if($t<time()){
                    $t = time()+$item['exprect_time']*60;
                }
                $item['is_follow']=$follow->getUserFollow($uid,$item['id']);
                $item['countDonw']= $t;
                $item['num']= 1;
                $item['next_time'] = date('Y-m-d H:i:s',$t);
                $data[]=$item;
            }
        }
        return $data;
    ...
    public function checkoutProductStock(int $uid, int $combinationId, int $cartNum = 1, string $unique = '')
    {
        ...
        if (!$StoreCombinationInfo) {
            throw new ValidateException('该商品已下架或删除-7');
        }
        ...
        if ($cartNum > $res['quota']) {
            throw new ValidateException('该商品库存不足f' . $cartNum);
        }
        ...
        if ($product_stock < $cartNum) {
            throw new ValidateException('该商品库存不足s' . $cartNum);
        }
        return $unique;
    }
    /**
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function checkCombinationStockV2(int $uid, int $combinationId, int $cartNum = 1, string $unique = '')
    {
        /** @var StoreProductAttrValueServices $attrValueServices */
        $attrValueServices = app()->make(StoreProductAttrValueServices::class);
        if ($unique == '') {
        ...
        if (!$StoreCombinationInfo) {
            throw new ValidateException('该商品已下架或删除-8');
        }
        /** @var StoreOrderServices $orderServices */
        $orderServices = app()->make(StoreOrderServices::class);
        $userBuyCount = $orderServices->getBuyCount($uid, 'combination_id', $combinationId);
        ...
        if ($cartNum > $attrInfo['quota']) {
          //  throw new ValidateException('该商品库存不足fd' . $cartNum);
        }
