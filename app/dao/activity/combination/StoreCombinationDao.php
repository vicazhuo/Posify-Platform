...
class StoreCombinationDao extends BaseDao
{
    ...
    {
        return $this->search($where)->where('is_del', 0)->with('getPrice')->with('getLuckly')
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function combinationListV2(array $where, int $page, int $limit)
    {
        return $this->search($where)->with('getLuckly')->page($page, $limit)->order('sort desc,id desc')->select()->toArray();
    }
    /**
     * 条件获取数量
     * @param array $where
