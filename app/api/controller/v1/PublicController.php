...
class PublicController
{
    ...
    public function test()
    {
        ...
        $where=[
           // ['cid','=',23],
           // ['luck_no','<',2],
            ['next_time','>','2022-07-14 14:20:21'],
            ['status','=',0]
        ];
        $list = $storePinkLucklyServices->getPinkLucklyList($where);
print_r($list);
        ...
        // $updateData=['luck_on'=>$luck_on,'next_time'=>$next_time];
        // $storeCombinationLucklyServices->saveData($data->id,$updateData);
//
//       if($services->open(2,13,17,0)){
...
//        /** @var PinkAllocationServices $PinkAllocationServices */
//           $PinkAllocationServices =app()->make(PinkAllocationServices::class);
//           $PinkAllocationServices->newRefund(267,3,25);

//        /** @var StoreOrderServices $services */
//        $services =app()->make(StoreOrderServices::class);
//        $services->coptyOrder(22,['uid'=>3,'nickname'=>'asdasd','avatar'=>'asdasdasdasdas']);


//        $orderCacheKey = 'pkallocation_wx274214558875254784';
//        if ($orderCacheKey) {
//            $data = CacheService::redisHandler()->get($orderCacheKey);
...
//            $res = $services->applyRefund((int)$data['order_id_key'], $uid, [], [], 1, (float)$data['total_price'], $refundData,0,$data);
//            if ($res)
//                return app('json')->successful('提交申请成功');
//        }
//        /** @var PayNotifyServices $payService */
//        $payService  = app()->make(PayNotifyServices::class);
//        $payService->wechatProduct('wx273856056780652544');

//        /** @var QueeServices $queeService */
//        $queeService  = app()->make(QueeServices::class);
//        $queeService->continuPintuan(['order_id'=>12312312321,'expiration_time'=>10000]);
    }
