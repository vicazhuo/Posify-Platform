...
Route::group('v2', function () {
    ...
    Route::group(function () {
        ...
        //定时请求接口
        Route::post('cron4414', 'v2.activity.StoreCombinationController/pink');
...
//        Route::get('get_today_coupon', 'v2.store.StoreCouponsController/getTodayCoupon');//新优惠券弹窗接口
        ...
        //关闭自动参团
        Route::post('orderAutoJoin','v2.order.StoreOrderController/orderAutoJoin')->name('orderAutoJoin');
        //清除搜索记录
        Route::get('user/clean_search', 'v2.user.UserSearchController/cleanUserSearch')->mame('cleanUserSearch');
        //用户余额导入佣金
        Route::post('commitChange','v2.user.UserSetingController/commitChange')->name('commitChange');
        //抽奖活动详情
        ...
        //活动---拼团返佣
        Route::get('combination/list', 'v2.activity.StoreCombinationController/lst')->name('combinationList');//拼团商品列表
        //参团
        Route::post('combination/joinPink','v2.activity.StoreCombinationController/follow')->name('follow');
        //自动跟团
        Route::post('combination/add_follow','v2.activity.StoreCombinationController/add_follow')->name('add_follow');
    ...
    //小叮当支付通知
    Route::group(function () {
        Route::get('notify/xddpay', 'v2.XddPayController/index')->name('index');//接受通知
