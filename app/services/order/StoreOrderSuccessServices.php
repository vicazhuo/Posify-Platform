...
class StoreOrderSuccessServices extends BaseServices
{
     ...
     * @return bool
     */
    public function paySuccessV2(array $orderInfo, string $paytype = PayServices::WEIXIN_PAY, array $other = [])
    {
        ...
        $resPink = true;
        if ($orderInfo['combination_id'] && $res1) {
            /** @var StorePinkServices $pinkServices */
            $pinkServices = app()->make(StorePinkServices::class);
            /** @var StoreOrderServices $orderServices */
            $orderServices = app()->make(StoreOrderServices::class);
            $resPink = $pinkServices->createPinkV2($orderServices->tidyOrder($orderInfo, true));//创建拼团
        }
        //订单支付成功后置事件
