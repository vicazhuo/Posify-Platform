...
class StoreCartServices extends BaseServices
{
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function checkProductStockV2(int $uid, int $cartNum, string $unique, int $type = 0, $productId, int $seckillId, int $bargainId, int $combinationId, int $advanceId)
    {
        ...
        switch ($type) {
            ...
            case 3://拼团
                ...
                [$attrInfo, $unique, $productInfo] = $combinationService->checkCombinationStockV2($uid, $combinationId, $cartNum, $unique);
                break;
            case 6://预售
    ...
    public function checkProductStock(int $uid, int $cartNum, string $unique, int $type = 0, $productId, int $seckillId, int $bargainId, int $combinationId, int $advanceId)
    {
        ...
        switch ($type) {
            case 0://普通
                ...
                $nowStock = $attrInfo['stock'];//现有库存
                if ($cartNum > $nowStock) {
                    throw new ValidateException('该商品库存不足y' . $cartNum);
                }
                if ($productInfo['is_virtual'] == 1 && $productInfo['virtual_type'] == 2 && $attrInfo['coupon_id']) {
                    /** @var StoreCouponIssueServices $issueCoupon */
                    $issueCoupon = app()->make(StoreCouponIssueServices::class);
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function setCartv2(int $uid, int $product_id, int $cart_num = 1, string $product_attr_unique = '', int $type = 0, bool $new = true, int $combination_id = 0, int $seckill_id = 0, int $bargain_id = 0, int $advance_id = 0)
    {
        if ($cart_num < 1) $cart_num = 1;
        //检测库存限量
        [$attrInfo, $product_attr_unique, $bargainPriceMin, $cart_num, $productInfo] = $this->checkProductStockV2($uid, $cart_num, $product_attr_unique, $type, $product_id, $seckill_id, $bargain_id, $combination_id, $advance_id);
    ...
    public function setCartNum($uid, $productId, $num, $unique, $type)
    {
        ...
        if (!$productServices->isValidProduct((int)$productId)) {
            throw new ValidateException('该商品已下架或删除-2');
        }
        ...
        if ($productServices->getProductStock((int)$productId, $unique) < $num) {
            throw new ValidateException('该商品库存不足u' . $num);
        }
